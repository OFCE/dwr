...
calc_sim_dust <- function(params, model, probs=c(0.025, 0.5, 0.975), history = NULL) {
  p <- params
  p <- dust_fix_params(p)
  m <- model$new(pars=p, step=1, n_particles = p$draws, n_threads = 2, seed = p$seed)
...
set_params <- function(inputs, globals, datas)
{
  ...
  p$tx_deriv_dep <- c(rep(inputs$ddep/10, 10), rep(0, periods-10))
  p$tx_deriv_po <- c(rep(inputs$dtpo/10, 10), rep(0, periods-10))
