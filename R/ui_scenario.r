            ...
            obss[[str_c(uuid_s, "_graph")]] <<- observeEvent(input[[stringr::str_c("graph_saved_", uuid_s)]], {
              ...
              if(!uuid_s %in% scn$uuid) {
                ...
                dandp <- dataandparams(
                  ...
                  periods = lubridate::year(input$end_year )- p$start_year,
                  draws = p$draws, 
                  globals = globals
                )
  ...
  output$dwn_scn_csv <- downloadHandler(
    ...
    content = function(file) {
      ...
      scn <- scenarii() |>
        filter(saved | actuel | preset, uuid != "") |>
        group_by(uuid) |>
        summarise(sim = list(first(sim)), uuid = first(uuid), country = first(country), nom_fr = first(nom_fr), .groups = "drop")
      
      data <- pmap_dfr(
        list(scn$sim, scn$uuid, scn$country, scn$nom_fr),
        ~ {
          ..1 |> mutate(uuid = ..2, country = ..3, name = ..4)
        }
      ) |>
