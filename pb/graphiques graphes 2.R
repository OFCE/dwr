...
############################
  ...
  sim_cible<- readRDS(file = "pb/sim_cible.rds")
...
graph_cible <- sim_cible|> filter(variable %in% c("og", "tcho","tpo","dettep"))|> mutate(year=2022+year)|> bind_rows(donnees_historiques) |>mutate(dstar = factor(100*dstar, levels=c("60", "100", "117")),q0.5=q0.5)

variante_cible<-ggplot(graph_cible)+
  geom_line(data = ~.x |> filter(year>=2022), aes(x=year, y=q0.5, col=dstar), size =1, linetype="dotted") + 
  geom_line(data = ~.x |> filter(year<=2022), aes( x=year, y=q0.5, col=dstar), size = 0.75) + 
  facet_wrap(vars(variable),scales="free",labeller=labeller(variable=noms_de_variables)) + 
  scale_color_manual(values=paletteer::paletteer_d("miscpalettes::brightPastel")[c(6,2,3)],labels=c("60%","100%", "117%"))
...
sim_ec<-readRDS(file = "pb/sim_ec.rds")
