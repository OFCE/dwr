...
ui <- function(req) {
  fluidPage(
    ...
    gfonts::use_font("source-sans-pro", "www/css/source-code-pro.css"),
    title = "debtwatchr",
          ...
          div(
            ...
            div(
              ...
              collapseUI(
                ...
                content = div(
                  airYearpickerWrapper(
                    inputId = "start_hist",
                    label = i18n$t("Historique"),
                    help_text = NULL,
                    value = "2007",
                    maxDate = as.character(max(globals$history$year)),
                    minDate = as.character(min(globals$history$year))
                  ),
                  airYearpickerWrapper(
                    inputId = "start_year",
                    label = i18n$t("Début simulation"),
                    help_text = NULL,
                    value = "2022",
                    maxDate = as.character(globals$years[[2]]),
                    minDate = as.character(globals$years[[1]])
                  ),
                  airYearpickerWrapper(
                    inputId = "end_year",
                    label = i18n$t("Fin simulation"),
                    help_text = NULL,
                    value = "2050",
                    minDate = as.character(globals$years[[2]] + 1),
                    max = "2100"
                  )
                )
              )
          ...
          div(
            ...
            div(
              ...
              p(
                ...
                p(tags$small("Debtwatch ©2021 Timbeau, Heyer, Aurissergues sous licence CeCIIL-B")),
                tags$hr()
              ),
              p(
                class = "intro-more",
...
server <- function(input, output, session) {
  ...
  observeEvent(list(input$country, querycount()), {
    ...
    start <- lubridate::year(input$start_year)
    hist <- lubridate::year(input$start_hist)
    periods <- lubridate::year(input$end_year) - start
    draws <- input$draws
  ...
  observeEvent(input$start_hist, {
    shinyjs::html(
      selector = "#start_hist_value output",
      html = substr(input$start_hist, 1, 4)
    )
  })
  observeEvent(input$start_year, {
    shinyjs::html(
      selector = "#start_year_value output",
      html = substr(input$start_year, 1, 4)
    )
  })
  observeEvent(input$end_year, {
    shinyjs::html(
      selector = "#end_year_value output",
      html = substr(input$end_year, 1, 4)
    )
  })
  ...
  simulation <- reactive({
    ...
    isolate({ # on isolate à partir de là comme ça pas de mauvaise surprise
      ...
      if (rrefresh & !f_uuid$ruleok) {
        ...
        pp$p <- calc_rule_params(globals = globals, params = pp$p, draws = 500)
      } else {
  ...
  observeEvent(input$language_fr, {
    ...
    # Translate intro
    shinyjs::html(
      selector = ".intro-content-wrapper",
      html = as.character(
        p(
          class = "intro-content",
          "Debtwatch permet de calculer la politique budgétaire pour atteindre une cible de dette.",
          tags$p(tags$small("Debtwatch ©2021 Timbeau, Heyer, Aurissergues sous licence CeCIIL-B")),
          tags$hr()
        )
      )
    )
    ...
    shinyjs::html(
      ...
      html = as.character(
        p(
          ...
          "Simulations depend on hypotheses that can be set in the left panel.",
          tags$p(tags$small("Debtwatch ©2021 Timbeau, Heyer, Aurissergues under CeCIIL-B")),
          tags$hr()
        )
      )
    )
